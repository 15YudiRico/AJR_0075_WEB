...
<?php
    ...
    $query2 = mysqli_query($con, "SELECT no_sim FROM customer where id_customer='$id_customer'") or die(mysqli_error($con));
    $data2 = mysqli_fetch_array($query2);

    if($data2[0]==NULL) {
        echo '<script>alert("Silahkan Melengkapi Data SIM Anda Terlebih Dahulu!"); window.location = "./profilCustomerPage.php"</script>';
    }else {
        ...
        <form action="./updateCreateTransaksiWithoutDriverPage.php?id='.$id.'" method="post">
            <div class="mb-3">
                ...
                <input class="form-control" name="tgl_waktu_mulai_sewa" id="tgl_waktu_mulai_sewa" placeholder="Tanggal Waktu Mulai Sewa" required type="datetime-local" aria-describedby="emailHelp">
            </div>

            <div class="mb-3">
                <label for="exampleInputEmail1" class="form-label">Tanggal Waktu Selesai Sewa</label>
                <input class="form-control" name="tgl_waktu_selesai_sewa" id="tgl_waktu_selesai_sewa" placeholder="Tanggal Waktu Selesai Sewa" required type="datetime-local" aria-describedby="emailHelp">
            </div>
