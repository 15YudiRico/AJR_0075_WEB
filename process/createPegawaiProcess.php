...
<?php
    ...
    if(isset($_POST['add'])){
        ...
        $pass_pegawai = password_hash($_POST['tanggal_lahir_pegawai'], PASSWORD_DEFAULT);
        ...
        if(mysqli_num_rows($query) != 0 || mysqli_num_rows($query2) != 0 || mysqli_num_rows($query3) != 0) {
            echo
                '<script>alert("Email telah digunakan!"); window.location = "../page/addPegawaiPage.php"</script>';
        }else {
            // Melakukan insert ke databse dengan query dibawah ini
            $query5 = mysqli_query($con, "INSERT INTO users(`name`, `email`, `password`, `created_at`) VALUES ('$nama_pegawai', '$email_pegawai', '$pass_pegawai', now())") or die(mysqli_error($con));

            $query6 = mysqli_query($con, "SELECT id FROM users WHERE email='$email_pegawai'") or die(mysqli_error($con));
            $data = mysqli_fetch_array($query6);
            $id = $data[0]; 

            $query4 = mysqli_query($con, "INSERT INTO pegawai(nama_pegawai, id_jabatan, tanggal_lahir_pegawai, jenis_kelamin_pegawai, alamat_pegawai, no_telepon_pegawai, pass_pegawai, email_pegawai, foto_pegawai, id) VALUES ('$nama_pegawai', '$id_jabatan', '$tanggal_lahir_pegawai', '$jenis_kelamin_pegawai', '$alamat_pegawai', '$no_telepon_pegawai', '$pass_pegawai', '$email_pegawai', '$foto_pegawai', '$id')") or die(mysqli_error($con)); // perintah mysql yang gagal dijalankan ditangani oleh perintah “or die”
